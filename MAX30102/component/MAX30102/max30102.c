...
#include <esp_timer.h>
#include "max30102.h"
//Nếu không có thiết bị, nó báo lỗi ngay để tránh crash chương trình.
...
esp_err_t max30102_readPartID(i2c_dev_t *dev) {
    ...
    //// Đọc thanh ghi ID (0xFF)
    I2C_DEV_CHECK(dev, i2c_dev_read_reg(dev, MAX30102_CHIP_ID_ADDR, &data, sizeof(data)));
    I2C_DEV_GIVE_MUTEX(dev);

    if (data == MAX30102_CHIP_ID)// ID thường là 0x15
     {
        return ESP_OK;
    }
    else {
        return ESP_ERR_NOT_FOUND;// không tìm thấy thiết bị
    }
}
...
esp_err_t max30102_initDesc(i2c_dev_t *dev, i2c_port_t port, gpio_num_t sda_gpio, gpio_num_t scl_gpio)
{
    ...
    dev->port = port;// gán cổng I2C
    dev->addr = MAX30102_SENSOR_ADDR;
    dev->cfg.sda_io_num = sda_gpio;// gán chân SDA
    dev->cfg.scl_io_num = scl_gpio;// gán chân SCL
...
// 2. Xóa các bit cũ tại vị trí mask (bit = 0)
    data = data & mask;
// 3. Ghi đè bit mới vào (thing)
    data = data | thing;
// 4. Ghi ngược lại vào cảm biến
    I2C_DEV_CHECK(dev, i2c_dev_write_reg(dev, reg, &data, sizeof(data)));
    I2C_DEV_GIVE_MUTEX(dev);

    return ESP_OK;
}
//Chọn chế độ chỉ đèn Đỏ (Red Only) hay cả 2 đèn (Multi-LED).
...
//Cấu hình phạm vi ADC (ADC Range), tốc độ lấy mẫu (Sample Rate) và độ rộng xung (Pulse Width)
...
//Tốc độ chụp (50, 100, ... 400 mẫu/giây).
...
//Thời gian đèn sáng trong mỗi lần chụp (càng lâu càng rõ nhưng tốn pin).
...
//Sắp xếp lịch trình bật đèn (Ví dụ: Slot 1 bật Đỏ, Slot 2 bật Hồng ngoại).
...
//Xóa sạch dữ liệu cũ trong cảm biến bằng cách reset con trỏ Đọc/Ghi về 0.
...
//Cho phép ghi đè khi bộ nhớ đầy (tạo vòng lặp).
...
//Hàm khởi tạo cảm biến với các tham số cấu hình cơ bản
...
//đọc FIFO xủa lý wrap ghép byte và lưu vào struct max30102_record
int16_t max30102_check(struct max30102_record *record, i2c_dev_t *dev) {
    ...
    if (readPointer != writePointer)
    {
        ...
        // Nếu 2 con trỏ lệch nhau -> Có dữ liệu mới đang nằm giữa chúng.
        numberOfSamples = (int16_t)(writePointer - readPointer);
        if (numberOfSamples < 0) numberOfSamples += 32; //Wrap condition

        //We now have the number of readings, now calc bytes to read
        //For this example we are just doing Red and IR (3 bytes each)
        int bytesLeftToRead = (int)(numberOfSamples * record->activeLEDs * 3); // Mỗi mẫu gồm 3 byte Đỏ + 3 byte IR (nếu dùng 2 LED)
...
//Chờ cảm biến có dữ liệu mới trong thời gian tối đa maxTimeToCheck (ms)
...
//Lấy dữ liệu cảm biến cho đèn LED tương ứng (1- Red, 2- IR)
